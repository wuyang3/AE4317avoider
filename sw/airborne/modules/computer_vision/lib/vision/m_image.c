 ...
 */
void image_yuv422_colorfilt(struct image_t *input, struct image_t *output, struct thres thres_o, struct thres thres_r, struct thres thres_b, struct count *cnt)
{
  uint16_t l = 0;
  uint16_t m = 0;
  uint16_t r = 0;
  ...
  for (uint16_t y = 0; y < output->h; y++) {
    for (uint16_t x = 0; x < output->w/3*1 ; x += 2) {
